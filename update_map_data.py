...
# --- CHAINS LIST ---
# Strict list. Removed "点都德". Only items mentioned or implied in MD processing will be tagged.
CHAINS = [
    "华星冰室", "翠华餐厅", "兰芳园", "Bakehouse", "达扬炖品", "炳胜品味", 
    "惠食佳", "陶然轩", "义顺牛奶公司", "澳洲牛奶公司", "何洪记", "池记", "再兴烧腊",
    "妈咪鸡蛋仔", "敏华冰厅", "棋哥", "泰昌饼家", "利苑酒家", "稻香", "佳佳甜品",
    "许留山", "满记甜品", "海皇粥店", "一粥面", "太兴", "大快活", "大家乐", "美心",
    "东海堂", "奇华", "荣华", "恒香", "鉅记", "Vission Bakery", "光荣冰室", "北角鸡蛋仔",
    "兴记"
]
...
PREDEFINED_LOCATIONS = {
    ...
    "德记": [23.1360, 113.2400], # Bailing Rd (Porridge)
    
    # --- Hong Kong (Strictly prioritize Central, Wan Chai, TST, CWB) ---
    "华星冰室": [22.2775, 114.1782], # Wan Chai (Heung Yop Rd)
    "金华冰厅": [22.3228, 114.1691], # Prince Edward (The Main One)
    "翠华餐厅": [22.2819, 114.1555], # Central (Wellington)
    "食光荣": [22.3180, 114.1700], # "光荣冰室" - TST (Hau Fook St) [22.2990, 114.1735] OR CWB [22.279, 114.182]. Let's pick CWB to diversify from TST. CWB: [22.2796, 114.1820]
    "金禾阁": [22.3800, 114.1950], # Sha Tin (User specified)
    "美好快餐": [22.3800, 114.1950], # Sha Tin (User specified)
    "源记": [22.2988, 114.1722], # TST (Granville Rd)
    "佳记餐厅": [22.2988, 114.1722], # TST
    "泰昌饼家": [22.2825, 114.1539], # Central (Lyndhurst Terrace)
    "瑞记": [22.2858, 114.1502], # Sheung Wan
    "科记咖啡餐厅": [22.2868, 114.1486], # Sheung Wan
    "金凤茶餐厅": [22.2751, 114.1728], # Wan Chai
    "新香园": [22.3300, 114.1610], # Sham Shui Po (Iconic main branch, no Central/TST branch usually)
    "利苑酒家": [22.2798, 114.1789], # Wan Chai
    "荣记饭店": [22.2783, 114.1802], # Wan Chai (Explicit)
    "莲香居": [22.2882, 114.1448], # Sheung Wan
    "稻香": [22.3956, 114.1963], # Fo Tan (Explicit)
    "端记茶楼": [22.3193, 114.1694], # Tai Mo Shan
    "莲香楼": [22.2842, 114.1538], # Central
    "牛阵": [22.2970, 114.1720], # TST
    "源記甜品": [22.2860, 114.1400], # Sai Ying Pun (Classic)
    "佳佳甜品": [22.3056, 114.1690], # Jordan (Iconic, near TST)
    "福元湯圓": [22.2885, 114.1932], # North Point
    "北角雞蛋仔": [22.2980, 114.1720], # TST Branch (Lee Keung Kee) [22.303, 114.171] -> TST (Nathan Rd/Hillwood Rd). Search says TST Nathan Rd.
    "沾仔記": [22.2818, 114.1552], # Central
    "廟街興記": [22.3094, 114.1702], # Temple St
    "華香園": [22.2974, 114.1691], # TST (Haiphong Rd)
    "媽咪雞蛋仔": [22.2980, 114.1725], # TST
    "合香園": [22.2980, 114.1690], # TST (Haiphong Rd Market)
    "德發牛丸": [22.2974, 114.1691], # TST
    "水記": [22.2841, 114.1538], # Central
    "生记": [22.2863, 114.1512], # Sheung Wan
    "荣记粉面": [22.2804, 114.1857], # CWB
    "文记": [22.3032, 114.1856], # Hung Hom
    "Kabo": [22.2980, 114.1720], # TST
    "英记面家": [22.2850, 114.1400], # Sai Ying Pun
    "达濠仔": [22.2870, 114.1410], # Sai Ying Pun (High St/Centre St)
    "卖奀记忠记": [22.2838, 114.1542], # Central
    "蛇王芬": [22.2824, 114.1540], # Central
    "面尊": [22.2820, 114.1550], # Central
    "华姐清汤腩": [22.2825, 114.1915], # Tin Hau
    "乐园鱼蛋粉": [22.2804, 114.1857], # CWB
    "庙街牛什": [22.3094, 114.1702], # Temple St
    "红磡鸡蛋仔": [22.3032, 114.1856], # Hung Hom
    "兴记": [22.3094, 114.1702], # Temple St (Claypot Rice/Oyster Cake). User says "Curry Tripe" which implies Sun Hing (Sai Ying Pun)? But MD also says "庙街兴记". "兴记 咖喱金钱肚" is separate line. Let's map to Temple St as it's the famous Hing Kee. Or "Hing Kee" TST. Search result says Hing Kee Temple St is Claypot. Sun Hing is dim sum (curry tripe). Let's map "兴记" to Temple St but note confusion? No, let's Map to Temple St.
    "新兴食家": [22.2850, 114.1400], # Sai Ying Pun (Curry tripe/Dim sum famous)
    "Bakehouse": [22.2820, 114.1530], # Central (Soho)
    "Vission Bakery": [22.2820, 114.1535], # Central (Soho)
    "Owls Choux": [22.2980, 114.1720], # TST
    "Shari Shari": [22.2820, 114.1545], # Central/Soho
    "Tokachi": [22.2830, 114.1530], # Central (Aberdeen St)
    "棋哥": [22.2780, 114.1800], # Wan Chai
    "甘堂": [22.2783, 114.1758], # Wan Chai
    "一乐": [22.2824, 114.1558], # Central
    "鏞記酒家": [22.2818, 114.1552], # Central
    "新桂香": [22.2700, 114.2380], # Chai Wan
    "国金轩": [22.2950, 114.1700], # TST (The Mira)
    "翠园": [22.2930, 114.1690], # TST (Star House/Harbour City)
    "旺角果栏": [22.3125, 114.1685], # Yau Ma Tei Fruit Market
}

# Updates to "食光荣"
# "食光荣" -> "光荣冰室", choose CWB branch: [22.2796, 114.1820]
PREDEFINED_LOCATIONS["食光荣"] = [22.2796, 114.1820]
# "北角鸡蛋仔" -> TST branch: [22.3032, 114.1718] (Nathan Rd)
PREDEFINED_LOCATIONS["北角雞蛋仔"] = [22.3032, 114.1718]
...
def get_coords_and_address(name, raw_line, city):
    ...
    # 3. District Fallback (Only use if no predefined match)
...
def get_district_from_coords(coords, city):
    if city == 'hk':
        if coords[1] < 114.145: return "西环/HKU"
        if 114.145 <= coords[1] < 114.160: return "中环/上环"
        if 114.160 <= coords[1] < 114.170: return "佐敦/旺角/油麻地" # Nathan Rd corridor
        if 114.170 <= coords[1] < 114.177: return "尖沙咀/湾仔"
        if 114.177 <= coords[1] < 114.188: return "铜锣湾/红磡"
        return "北角/其他"
    else:
